...
import TabNavigation, { SensorType } from "./components/TabNavigation/TabNavigation";
import { ChartLine, Droplets, Gauge, RefreshCcw, SunMedium, Thermometer } from "lucide-react";
...
export default function HistoricoPage() {
  ...
  const sensorConfig = {
    temperature: { color: '#ef4444', name: 'Temperature (°C)' },
    humidity: { color: '#3b82f6', name: 'Humidity (%)' },
    luminosity: { color: '#f59e0b', name: 'Luminosity (lux)' },
    pressure: { color: '#10b981', name: 'Pressure (hPa)' },
  };

  const dataChar = Array.from({ length: 20 }, (_, index) => ({
    timestamp: `2025-06-06T14:${34 - index}:00Z`,
    temperature: 20 + Math.random() * 10,
    humidity: 30 + Math.random() * 20,
    luminosity: 100 + Math.random() * 300,
            ...
            <div className="bg-white dark:bg-gray-900 p-4 rounded-sm shadow border border-gray-200 dark:border-gray-700">
              <div className="flex flex-col gap-4">
                ...
                <ResponsiveContainer width={800} height={500} >
                  <AreaChart data={dataChar} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                    <defs>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-sm shadow border border-gray-200 dark:border-gray-700 max-h-150 flex flex-col gap-3">
            <div className="flex flex-col border-b border-gray-200 dark:border-gray-700 pb-3 shrink-0 p-4">
              ...
              <span className="text-gray-400 text-sm">
                actualizacion: {"06/06/2025 14:34"}
              </span>
            </div>
            {/* Contenido historico */}

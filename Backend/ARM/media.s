...
.data
inputfile: .asciz "datos.txt" // Cambia el nombre aquí si quieres otro archivo
newline: .asciz "\n"

.text
_start:
    // open file (filename in data section)
    mov x0, -100        // AT_FDCWD (relative to current directory)
    ldr x1, =inputfile  // filename address
    mov x2, 0           // O_RDONLY 
...
convert:
    udiv x5, x5, x6     // calculate average (media)
